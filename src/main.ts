class CopyElement extends HTMLElement {
  button: HTMLSpanElement | undefined

  constructor () {
    super()

    const shadowRoot = this.attachShadow({ mode: 'open' })
    this.button = this.createButtonElement(this.theme)
    shadowRoot.appendChild(this.button)

    this.button.addEventListener('click', () => {
      const tooltip = this.createTooltipElement(this.getAttribute('tooltip-position') ?? 'top', this.getAttribute('tooltip-text') ?? 'copied!')
  ...
  createTooltipElement = (tooltipPosition: string, tooltipText: string): HTMLSpanElement => {
    ...
    if (tooltipPosition === 'top') {
      tooltip.style.marginTop = '-28px'
      tooltip.style.marginLeft = '-38px'
    }
    ...
    if (tooltipPosition === 'right') {
      tooltip.style.marginTop = '-1px'
      tooltip.style.marginLeft = '4px'
    }

    if (tooltipPosition === 'left') {
      tooltip.style.marginTop = '-1px'
      tooltip.style.marginLeft = '-76px'
    }
